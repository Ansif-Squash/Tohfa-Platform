...
function WrenchIcon({ size = 20, color = P.primary }: { size?: number; color?: string }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <Path
...
export interface EditToolScreenProps {
  tool?: Partial<ToolFormData> | undefined;
  onNavigateBack: () => void;
  onSave?: ((data: ToolFormData) => void) | undefined;
...
export function EditToolScreen({
  ...
  onSave,
  onRemove,
}: EditToolScreenProps): React.JSX.Element {
  const [name, setName] = useState(tool?.name || 'Knapsack Sprayer');
  const [purchaseDate, setPurchaseDate] = useState<Date | null>(
    tool?.purchaseDate ? new Date(tool.purchaseDate) : new Date('2025-01-04')
  );
  const [showPurchaseDateCalendar, setShowPurchaseDateCalendar] = useState(false);
  const [serviceInterval, setServiceInterval] = useState(tool?.serviceInterval || '90');
    ...
    <SafeAreaView style={styles.screen}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerTextWrap}>
          <Text style={styles.headerTitle}>Edit Tool</Text>
      ...
      >
        ...
        <View style={styles.categoryBanner}>
          <View style={styles.categoryIconCircle}>
            <WrenchIcon size={20} color={P.primary} />
          </View>
