...
export function EditAnimalScreen({
  ...
  const [tag, setTag] = useState(initialAnimal?.tag || 'C-014');
  ...
  const [dob, setDob] = useState(initialAnimal?.dob || '02 Mar 2022');
  const [source, setSource] = useState<'born_on_farm' | 'purchased'>(
    initialAnimal?.source || 'purchased',
  );
  const [purchaseDate, setPurchaseDate] = useState(initialAnimal?.purchaseDate || '10 Jul 2022');
  ...
  const handleSave = () => {
    ...
    const payload: AnimalFormData = {
      id: initialAnimal?.id || 'a1',
      tag: tag.trim(),
      name: initialAnimal?.name || 'Lakshmi',
      species,
      gender,
      breed: breed.trim(),
      dob,
    ...
    if (onSave) {
      ...
      Alert.alert(
        'Animal Updated',
        `Animal ${tag.trim()} (${breed.trim() || species}) details have been saved.`,
          ...
          <View style={styles.headerLeft}>
            ...
            <View style={styles.headerTitleCol}>
              ...
              <Text style={styles.headerSubtitle} numberOfLines={1}>
                {initialAnimal?.name || 'Lakshmi'} · {tag}
              </Text>
            </View>
          </View>
        ...
        >
          ...
          <View style={styles.formContainer}>
            ...
            <View style={styles.fieldGroup}>
              ...
              <Text style={styles.ageHelperText}>≈ 4 yr old</Text>
            </View>
