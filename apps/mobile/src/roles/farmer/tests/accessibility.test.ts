...
describe('Farmer Mobile Accessibility Pass (S-46)', () => {
  ...
  it('verifies core controls have accessibilityRole or accessibilityLabel in source', () => {
    ...
    // srcDir here is this role's own JS tree (apps/mobile/src/roles/farmer),
    // 2 levels deeper under the repo root than the old standalone
    // apps/farmer-mobile/src was, so reaching repo-root-level packages/ now
    // needs 5 '../' instead of 3.
    const filesToCheck = [
